...
bool monolithic() {
    bool answer, fifthcriterion;
    answer = false;
    fifthcriterion = true;
	if (supp < 2) return false;
	if (support[1] < 0.5 * (support[0] + support[supp - 1])) {
		cout << "f is monolithic by the first criterion, ";
		answer= true;
	}
	if (supp > 3 && support[1] <= 0.5 * (support[0] + support[supp - 1])) {
		cout << "f is monolithic by the second criterion, ";
		answer= true;
	}
	if (support[supp - 2] > 0.5 * (support[0] + support[supp - 1])) {
		cout << "f is monolithic by the third criterion, ";
		answer= true;
	}
	if (supp > 3 && support[supp - 2] >= 0.5 * (support[0] + support[supp - 1])) {
		cout << "f is monolithic by the fourth criterion, ";
		answer= true;
	}
	int diff = support[1] - support[0];
	for (int i = 2; i < supp; i++) {
		if (support[i] - support[i - 1] >= diff) fifthcriterion = false;
	}
    if(fifthcriterion) cout << "f is monolithic by the fifth criterion, ";
   if (answer || fifthcriterion) cout << "so ";
	return (answer || fifthcriterion);
}

bool crowdmath() {
    bool returnvalue = false;
	if (supp > 3 && numodd == 1) {
		cout << "f has only one odd degree term, ";
        returnvalue=true;
	}
	if (numodd == supp) {
		cout << "f has no even degree terms, ";
		returnvalue= true;
	}
    if (returnvalue) cout << "so ";
	return returnvalue;
}
...
bool bonciocat() {
	for (int i = maxnum + 1; i <= maxnum + 100; i++) {
		...
		if (p.first != -1 && derivativeNotDivide(i, p.first)) {
			cout <<"f("<<i<<") = " << v <<" = " << p.first << "^" << p.second<<" is a prime power, and "<<p.first<<" doesn't divide f'("<<i<<"), so";
			return true;
		}
	}
	return false;
